...
| Время построения индекса, с | 0.160432 |
| Запрос | `love story` |
| Число результатов AND | 64 |
| Время AND, с | 0.00000702 |
| Число результатов OR | 398 |
| Время OR, с | 0.00001785 |
| Время полного перебора, с | 0.09846586 |
| Ускорение за счёт индекса | 11627x |
...
4. Индекс против перебора. Поиск по индексу оказался быстрее полного просмотра корпуса
   примерно в 11627 раз. Причина в том, что перебор заново токенизирует все
   1000 документов при каждом запросе, а индекс лишь берёт готовые posting list и выполняет
   операцию над множествами. Цена — время построения индекса (0.1604 с), память под
