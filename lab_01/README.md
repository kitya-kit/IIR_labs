...
| Время построения индекса, с | 0.157677 |
| Запрос | `love story` |
| Число результатов AND | 64 |
| Время AND, с | 0.00001300 |
| Число результатов OR | 398 |
| Время OR, с | 0.00002690 |
| Время полного перебора, с | 0.18084275 |
| Ускорение за счёт индекса | 23219x |
...
4. Индекс против перебора. Поиск по индексу оказался быстрее полного просмотра корпуса
   примерно в 23219 раз. Причина в том, что перебор заново токенизирует все
   1000 документов при каждом запросе, а индекс лишь берёт готовые posting list и выполняет
   операцию над множествами. Цена — время построения индекса (0.1577 с), память под
